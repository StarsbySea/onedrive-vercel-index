...
const VideoPreview: FC<{ file: OdFileObject }> = ({ file }) => {
  ...
  const thumbnail = `https://cache.ptufdc.com${asPath}&size=large${hashedToken ? `&odpt=${hashedToken}` : ''}`

  // We assume subtitle files are beside the video with the same name, only webvtt '.vtt' files are supported
  const vtt = `${asPath.substring(0, asPath.lastIndexOf('.'))}.vtt`
  const subtitle = `https://cache.ptufdc.com${vtt}${hashedToken ? `&odpt=${hashedToken}` : ''}`

  // We also format the raw video file for the in-browser player as well as all other players
  const videoUrl = `https://cache.ptufdc.com${asPath}${hashedToken ? `&odpt=${hashedToken}` : ''}`
